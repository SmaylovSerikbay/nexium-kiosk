...
5. Запустить приложение — оно само:
   - назначит `MainActivity` постоянным системным Home-приложением, поэтому Android
     будет автоматически запускать его после включения/перезагрузки без окна выбора лаунчера;
...
6. Проверить автозапуск:
   ```bash
   adb reboot
   ```
   После завершения загрузки (и разблокировки, если на устройстве настроен PIN/пароль)
   приложение должно открыться само и войти в Lock Task Mode.
